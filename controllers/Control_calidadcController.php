...
class Control_calidadcController{

    public function __construct() {
        $this->name = "control_calidadc";
        $this->title = "Calidad";
    ...
    public function edit($id){
        $data['get'] = $this->model['control_calidadc']->find($id);
        //var_dump($data['get'][0]);
        if (exists($data['get'])) {
    ...
    public function enable($id){
        ...
        if($this->model['equipo']->update($dataequipo)){
            if ($this->model['control_calidadc']->update($data)) {
    ...
    public function store(){
        $data = validate($_POST, [
            'equipos_id' => 'required|toInt|unique:control_calidadc'.$this->ext,            
        ...
        if($this->model['control_calidadc']->find_by(['equipos_id' => $data['equipos_id']])){
            ...
            if ($this->model['control_calidadc']->store($data)) {
    ...
    public function destroy(){ 
        ...
        if($this->model['control_calidadc']->destroy($data)){
            redirect('?c=' . $this->name);
        } else {
            Flash::error(setError('002'));
        }        
