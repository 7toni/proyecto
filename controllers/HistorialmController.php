<?php
Session::logged();
$_SESSION['script']= false;

class HistorialmController{

    public function __construct(){        
        $this->name ="historialm";
    ...
    public function edit($id){
        $data['get']= $this->model['historialm']->find($id);        
        if(exists($data['get'])){
            $data['usuarios']=$this->model['usuario']->find_by(['plantas_id'=>Session::get('plantas_id'),'activo'=>'si'],'view_usuarios');
    ...
    public function store(){
        ...
        if($this->model['control_calidad']->find_by(['equipos_id' => $data['equipos_id']])){
            ...
            if($this->model['historialm']->find_by(['equipos_id' => $data['equipos_id'],'fecha'=> $data['fecha'],'responsable'=>$data['responsable']])){
                Flash::error(setError('015'));
            }
            else{
                if($this->model['historialm']->store($data)){
                    $historial=$this->model['historialm']->find_by(['equipos_id' => $data['equipos_id'],'fecha'=> $data['fecha'],'responsable'=>$data['responsable']]);
    ...
    public function update(){
        ...
        if($this->model['control_calidad']->find_by(['equipos_id' => $data['equipos_id']])){
            /* Preguntamos si existe ese registro con el mismo equipo, fecha y responsable, si existe muestra una alerta  */
            if($this->model['historialm']->find_by(['equipos_id' => $data['equipos_id'],'fecha'=> $data['fecha'],'responsable'=>$data['responsable'],'comentario'=>$data['comentario']])){
                Flash::error(setError('015'));
            }
            else{
                if($this->model['historialm']->update($data)){
                    if(isset($_POST['actualizarm'])){
                        $historial=$this->model['historialm']->find_by(['equipos_id' => $data['equipos_id'],'fecha'=> $data['fecha'],'responsable'=>$data['responsable'],'comentario'=>$data['comentario']]);
                        $control_calidad=$this->model['control_calidad']->find_by(['equipos_id' => $data['equipos_id']]);
    ...
    public function destroy(){
        ...
        if($this->model['historialm']->destroy($data)){
            redirect('?c=' . $this->name);
        } else {
            Flash::error(setError('002'));
        } 
